...
Leeds is situated in the West Yorkshire county of England, occupying 551.7 square kilometres of land area.
It has a population size of close to 800,000 (cite ONS) and a population density of about 1,450 residents per
square kilometres. Leeds is divided into 107 Middle Layer Super Output Areas (MSOAs), which are further
divided into 482 Lower Layer Super Output Areas (LSOAs). Each LSOA has an average population size of
about 1,600.

In terms of transportation, Leeds is well-connected by air, rail and roads. It is accessible by several major
motorways such as the M1 and A62. Congestion is a major problem in Leeds; Leeds was ranked 9th in the
UK’s most congested urban areas in 2018 (cite). On the road safety front, pedestrians and cyclists were
over-represented in accidents in Leeds. Based on the 2011 census, 15.6% of the surveyed population walk
to work, but only 2.3% cycle (cite). However, pedestrians and cyclists respectively accounted for 19.0% and
16.9% of more than 9,000 accident casualties between 2014 and 2018.

## 1.1 Scope

This report analyses road safety of pedestrians and cyclists in Leeds in terms of four categories: who, when,
where and how. We will first articulate the methodologies used in understanding, preparing, cleaning and visualising the data as part of exploratory data analysis (EDA) in Section 2. In Sections 3 to 6, we will
perform the analyses in each of the four categories and discuss the results. In Section 7, we will propose
policy recommendations to improve road safety for pedestrians and cyclists. We will discuss limitations of
our analyses in Section 8 before proposing areas for future research in Section 9. All analyses underlying
this report were performed using the R statistical software and are reproducible using the code available at
. . .
...
We will be using accidents and casualties data from the `stats19` package for the years 2014 to 2018. This
five-year period was chosen as it was deemed sufficient to produce meaningful, relevant and current insights.
In analysing geospatial distribution of accidents, we will be using Leeds’ highway data from the `osmdata`
package. We will also be using origin-destination data of Leeds from the `pct` package to derive walking and
cycling rates in Leeds. The datasets required in this report are also available at . . .
...
Before analysing the data, it was important to first understand the contents of the raw datasets before
pre-processing them into forms suitable for subsequent analyses. This section will explain these stages in
the context of the raw accident and casualties data from the `stats19` package.

## 3.1 Understanding the Raw Datasets

A useful way to understand the raw datasets was to first look at the column headings. The
`stats19_variables` function provided brief descriptions of the variables. Since these raw datasets
would later be concatenated, it was important that their column names matched. We found that for the raw
accidents datasets, the names of all of the 33 columns matched. However, for the raw casualties datasets,
it was found that the 2014 dataset contained 15 variables, but the rest contained 16. The missing variable
was `casualty_imd_decile`, which could have been recorded only from 2015 onwards.

In addition, we used the `str()` function to determine the classes of each variable as well as their values.
Doing so allowed us to identify variables which required conversion of classes. For example, the variable
`accident_severity` was of `character` class, but given that it is a catagorical variable, it had to be converted
to the `factor` class.
...
Having gained an understanding of the datasets, the next step was to prepare them for our analyses. We
filtered records for Leeds using the `filter()` function. For each year, we used the `left_join()` function
to join the raw accident data to the raw casualties data, using accident index as the joining key. Next, we
concatenated the datasets for the individual years into a main dataframe using the `bind_rows()` function.

Data cleaning is also an important part of data preparation. The raw datasets from the `stats19` package had
been cleaned and formatted to a large extent. Notwithstanding, we found that a minority of the variables
contained missing data. One of them was the variable `time`, which had five `NA` values. These missing values
were imputed with the median^[Median was chosen over mean as it is typically less sensitive to outliers.] of the non-missing values1. The variable `lsoa_of_accident_location` was
also found to have 58 `NA` values. Using `tmap` and the LSOA boundaries data, we manually inspected accident
locations corresponding to each missing value and assigned the nearest LSOA to each missing value.
...
An important part of data preparation is data cleaning. In a scenario that the raw datasets needs to be cleaned, one useful method is to visualise possible outliers in data through boxplots. These outliers may bedue to human errors and therefore need to be removed. The raw datasets which we had obtained from the `stats19` package had been cleaned and formatted to a large extent.  Notwithstanding, we used the `plot_missing()` function from the `DataExplorer` package and found that a minority of the variables still contained missing data. One of them is the variable `time`, which contained a total of five `NA` values. There are several ways to deal with such missing data, including deletion, imputation or filling them with zeroes. In this instance, we imputed the missing values of the `time` variable with the median of the non-missing values and median was chosen over mean because it is less sensitive to outliers.     
```{r #pre_process_1}
...
# There are 5 NAs in the column 'time'. We need to deal with this since we will be analysing accidents by time of hour.

# What is the median time of accident (in hours) in each year?
df_total_ac_leeds %>% 
  filter(!is.na(time)) %>% 
...
# Replace the 5 NA values with the median value of time_hour, 15.
df_total_ac_leeds <-
  ...
  select(c("accident_index", "date", "year", "month", "day", "time", "time_hour", 
           "time_minute", "day_of_week", "accident_severity", "number_of_casualties", 
           "casualty_severity", "casualty_type", "casualty_class", "sex_of_casualty", 
           "age_of_casualty", "first_road_class", "road_type", "speed_limit", "second_road_class", 
           "pedestrian_location", "pedestrian_movement", "location_easting_osgr", "location_northing_osgr", 
           "lsoa_of_accident_location", "number_of_vehicles", "first_road_class", "road_type", 
           "speed_limit", "junction_detail", "light_conditions", "weather_conditions", "road_surface_conditions")) # Select the desired columns only
  
# Convert relevant columns to factor class
df_total_ac_leeds <-
  df_total_ac_leeds %>% 
...
# Regroup casualty types into six main categories
...
# Out of 11943 records, there are many which correspond to the same accident event. Therefore, we need to filter out the duplicates. 
df_total_ac_leeds_cleaned = 
df_total_ac_leeds %>% # Start with the main dataframe 
  distinct(accident_index, .keep_all = TRUE) %>% # Only keep records of distinct accidents
  filter(!is.na(lsoa_of_accident_location)) # There are 42 records with NAs in the lsoa column. We will remove them. 

 [1] "20141316H1029" "20151328E1461" "2014130018623" "20171346E0216" "2014130030345" "2014131830367"
 [7] "20141318V0571" "20151325L0873" "20141312I0039" "2014130048209" "2014130043327" "20171349E0197"
[13] "2014130001153" "20171343E0213" "20171341E0023" "20141318G0660" "2014130004811" "20151328D1274"
[19] "20151327M1604" "20141317L1112" "2014131351484" "20171342E0246" "20151326R0374" "20141317O1333"
[25] "20141312H1210" "2014130008934" "2014131590760" "2015132860986" "20151321A0339" "2014130047033"
[31] "2014130050974" "2014130033332" "2014130035786" "2014131770761" "20171343E0234" "2014131730978"
[37] "2014130034085" "2014131BH1330" "2015132AI0812" "2014130007139" "2014130055061" "2015132CQ0615"

 [1] "E01011578" "E01011580" "E01011457" "E01011465" "E01011462" "E01011687" "E01011381" "E01011381"
 [9] "E01011558" "E01011718" "E01011407" "E01011296" "E01011396" "E01011302" "E01011411" "E01011628"
[17] "E01032495" "E01011468" "E01011368" "E01011523" "E01032503" "E01011610" "E01011738" "E01011529"
[25] "E01011366" "E01011677" "E01011677" "E01011293" "E01011481" "E01033005" "E01011446" "E01011443"
[33] "E01032946" "E01011415" "E01011423" "E01011430" "E01011430" "E01011422" "E01011435" "E01033006"
[41] "E01011422" "E01011427"
...
Out of a total of 48 variables, we selected 28 (?) and removed the rest as they were not needed for our subsequent analyses. We also used functions in the `lubridate` package to extract the year, month and day from the `date` variable. As some variables were categorical in nature but of the `character` class, they had to be converted to the `factor` class accordingly. There were 21 different types of casualties in the dataset altogether, but since our analyses would not require such fine-grained distinctions, we recategorised them into six main groups. We also found that there were multiple records in the dataset which corresponded to a single accident event, possibly due to accidents with multiple casualties. Thus, we filtered out the duplicates to retain only distinct accident events in the dataset. We also removed rows for which the LSOA information was missing. 

```{r load_df_total_ac_leeds_cleaned, eval=TRUE}

df_total_ac_leeds <- read.csv("Data/df_total_ac_leeds.csv")
df_total_ac_leeds_cleaned <- read.csv("Data/df_total_ac_leeds_cleaned.csv")

```

## 3.3 Exploratory Data Analysis

Part of the EDA is to extract summary statistics and general trends from the data. We used the `summary()` function to obtain a summary of numerical variables, such as `age_of_casualty` and `number_of_vehicles`. We plotted a time-series graph to understand the trend of monthly number of accidents in Leeds from 2014 to 2018 (see Figure 1). It appeared that there was, to some extent, a seasonal pattern to the occurence of accidents in Leeds over this five-year period. The number of accidents typically increased at the beginning of the year, fluctuated over the next few months, and decreased towards the end of the year.  
...
df_total_ac_leeds_cleaned %>% 
  ...
  labs(subtitle="Number of Accidents", x = "Year", y = "Number of Accidents") + 
...
df_total_ac_leeds %>% 
  ...
  geom_col(show.legend = FALSE, width = 0.6) +
  scale_color_discrete(palette = "Paired") +
  labs(subtitle="Number of Casualties", x = "Year", y = "Number of Casualties") + 
  geom_text(aes(label = count), vjust=-0.30, size=3))
...
Figure 2 shows the overall numbers of accidents and casualties from 2014 to 2018. Both sub-figures in Figure 2 follow similar trends since the number of casualties and number of accidents are closely associated. The casualties numbers would reasonably be higher than accident counts because some accidents would involve multiple casualties. From Figure 2, it appears that the roads in Leeds are becoming safer, with steady declines in the number of accidents and casualties since 2015. The accident count in 2018 was 25% lower than that in 2015. A breakdown of these numbers by severity reveals further insights (see Figure 3). The decline in overall accidents since 2015, as we saw in Figure 2, was mainly contributed by the decrease in slight accidents, and to a lesser extent, serious accidents. Fatal accidents, however, had been on an increasing trend since 2016. 
...
(figure_3 <-
  ...
  # distinct() %>% # Only distinct accidents 
...
In this section, we focus on the question, "who were most at risk of accidents?". In answering this question, the approach was to understand the profiles of accident casualties based on past data. 

## 4.1 Methodology

In the interest of protecting data privacy, limited demographic information about accident victims was provided in the dataset. Nevertheless, using available information, we plotted histograms and boxplots to analyse the distributions of pedestrian and cyclist casualties by age and gender. (why histograms and boxplots?) 
...
```{r #save_figure_5a_and_5b}

grid.arrange(figure_5a, figure_5b, nrow = 1, ncol = 2)

ggsave("Figures/Figure_5.png", 
       arrangeGrob(figure_5a, figure_5b, nrow = 1, ncol = 2),
       width = 12, height = 4)

```

```{r display_figure_5, eval=TRUE, echo=FALSE, include=TRUE, fig.cap="Graphical summaries of profiles of pedestrian and cyclist casualties"}

include_graphics("Figures/Figure_5.png")

```

Despite having limited demographic information, the plots in Figure 5 nonetheless provide valuable insights about the profiles of accident casualties. For cyclists, there were significantly more male casualties than female. The distribution of male cyclist casualties appears to be bimodal, that is, having two peaks -- one at around 24 years old and another at around 40. The former could be attributed to male student population at the universities who choose to cycle to school, whereas the latter could be predominantly avid sports cyclists who cycle for either competitive or recreational purposes. The peak age for female cyclist casualties is less apparent, although it seems to be around mid-20s. One similarity is that the age distributions for both male and female cyclist casualties taper off at 75. 

Unlike cyclist casualties, pedestrian casualties had more equal distribution of male and female. The age distributions for male and female pedestrian casualties are both positively skewed, with peaks at 12 and 14 years respectively. These could be mainly high school students who get into accidents during their journeys either to or from schools, suggesting that more can be done by parents and schools to educate young children on road safety. The histogram also shows that, unlike cyclists, the age distribution only tapers off in the late-80s or 90s. This implies that there were many elderly pedestrians who were susceptible to accidents. 

The boxplots for cyclist casualties show that the median age of male casualties, shown by the middle black line in the box, was slightly higher than that of female casualties, although the spread of age in the middle 50% of data, measured by the inter-quartile range (IQR) and shown by the height of the box, were roughly similar. Pedestrian casualties, on the other hand, have lower median ages than cyclist casualties, suggesting that younger people tend to be victims of accidents as pedestrians. Another difference is that pedestrian casualties have higher IQR, which means that the range of age of victims as pedestrians is wider than as cyclists.  

# 5. The 'When': Temporal Distribution of Accidents

In this section, we seek to answer the question, "When were accidents most likely to occur?" In answering this question, we will analyse temporal patterns of accident involving pedestrians and cyclists by months, days and hours. 

## 5.1 Methodology

To aid us in uncovering temporal patterns, we visualised the data using heatmaps. In general, heatmaps are a useful tool to visualise relative volumes of events within the dataset. The effectiveness of heatmaps stems from their abilities to draw the viewer's attention to specific trends and areas of interest through the use of colours and shades. Heatmaps are extremely powerful in our analysis because the darker shades indicate the times when it was most accident-prone for pedestrians and cyclists. 

We created two heatmaps, one for visualising the number of accidents by days of a week and months of a year, and another for visualising the number of accidents by days of a week and hours of a day. Before creating these heatmaps, the data first needed to be in the correct form. For instance, it was necessary to specify the levels of the variables `day_of_week` and `month`, to ensure that the sequence of months and days are accurately reflected in the resulting heatmaps. The `geom_tile()` function was then used together with `facet_grid()` and `facet_wrap()` to display the heatmaps for pedestrian and cyclist accidents together. The detailed code is shown in the R Markdown file.   
...
figure_6 + ggsave("Figures/Figure_6.png", width = 12, height = 3)

```

```{r display_figure_6, eval=TRUE, echo=FALSE, include=TRUE, fig.cap="Heatmap showing the number of pedestrian and cyclist accidents, by day and month"}
...
(figure_7 <- 
  ...
  # scale_x_reverse(breaks = break_hour,
  #              labels = c("00:00", "04:00", "08:00", "12:00", "16:00", "20:00", "24:00")) +
  theme_bw() + theme_minimal() + 
  # facet_grid(~cas_type) +
...
# Save the plot
figure_7 + ggsave("Figures/figure_7.png", width = 8, height = 4)

```

```{r #save_figure_7a_and_7b}

# grid.arrange(legend, fig6, fig7, fig8, nrow = 1, ncol = 4, widths = c(0.9, 2.1, 2.1, 2.1))

ggsave("Figures/Figure_7.png", arrangeGrob(figure_7a, figure_7b, nrow = 1, ncol = 2),
       width = 8, height = 6)

```

```{r display_figure_7, eval=TRUE, echo=FALSE, include=TRUE, fig.cap="Heatmap showing the number of pedestrian and cyclist accidents, by hour and day", out.width="60%"}

include_graphics("Figures/Figure_7.png")

```

## 5.2 Results and Discussions

The two heatmaps are shown in Figures 5 and 6. Figure 6 reveals several interesting patterns. November, December and January seem to be dangerous months for cyclists. During these winter months, the weather is typically colder, and the roads wet and slippery. Cyclists are exposed to such harsh weather elements and these conditions make them more susceptible to accidents. On the other hand, the safest months for cyclist are April and August. 

For pedestrians, the last two weeks of the months of September seem to have recorded most accidents. This time also coincides with the beginning of the academic year for University students, which means this is the time of the year in Leeds when there is an increase in the  population of either returning students or new international students.   Unlike cyclists, the winter months tend to be relatively safer for pedestrians, which seems reasonable because people might tend to go out less due to the cold weather. 

Figure 7 reveals temporal patterns in terms of hours of day and day of the week. A common pattern across both groups is that the numbers of accidents were low during the early hours of day. This is reasonable because these are generally the non-waking hours. Notwithstanding, there were more accidents involving pedestrians between midnight and 4.00am on weekends than on weekdays. A plausible explanation is that more individuals might go out to meet their friends and celebrate the weekend on Friday and Saturday nights till early hours the next day.

Cyclists seemed to be most at risk of getting into accidents during two distinct periods on weekdays -- 7.00am to 9.00am and 4.00pm to 6.00pm. This could be because many individuals choose to cycle to and fro work or school. These accident-prone timings are reasonably consistent with typical work or school hours on weekdays. For pedestrian, the most accident-prone timings on weekdays are between 3.00pm and 6.00pm. Interestingly, pedestrians seemed to be particularly accident-prone on Saturdays between 4.00pm and 5.00pm.

# 6. The 'Where': Geospatial Distribution of Accidents 

In this section, we will analyse the geospatial distribution of accidents involving pedestrians and cyclists, and assess if there were any areas in Leeds which are particularly accident-prone for each group. Such analyses have significant real-world implications as they can influence policy-making. Local councils and authorities are better informed in assessing and prioritising resources for road infrastructure improvement works.  
...
```{r display_figure_9, eval=TRUE, echo=FALSE, include=TRUE, fig.cap="Geospatial distribution of top 10 accidents location and stretches of roads for pedestrians and cyclists by LSOAs", out.width="60%"}

include_graphics("Figures/figure_9.png")
...
```{r df_simple_model_results, eval=TRUE, include=TRUE}
...
```{r display_figure_4, eval=TRUE, echo=FALSE, include=TRUE, fig.cap="Proportion of Serious and Fatal Accidents in Leeds by Casualty Type, 2014-2018", out.width="60%"}
