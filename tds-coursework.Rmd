...
While an analysis by LSOAs is useful in allowing local authorities to focus on more accident-prone regions, it gives limited information on stretches of roads where pedestrians and cyclists were most susceptible. The second approach was therefore to analyse the geospatial distribution of accidents by road networks. The key objective was to determine the top 10 roads in Leeds which recorded the most accidents for pedestrians and cyclists. The different categories of highways in Leeds were first downloaded from OpenStreetMap using the `osm` package and these individual datasets were then concatenated. The resulting dataset was an simple features (sf) object with linestrings representing stretches of roads. 

Spatial joins were performed using the `st_is_within()` function to link accident point data to the nearest stretch of road within a distance of 5 metres. Names and references of roads from the OpenStreetMap data were essential in this part of the analysis to identify specific road stretches which were most accident-prone. Unfortunately, as the data from OpenStreetMap is crowd-sourced, missingness of data was significantly rampant. 15.8% of the 3,268 accident records for pedestrians and cyclists had to be filtered out due to missing road names. This was the main limitation in this analysis. 
...
Our findings from this analysis can nevertheless influence policies to improve road safety. For instance, given that Harehills Lane was the only road in the top 5 lists that does not lead to the city centre but yet recorded high pedestrian accidents, local authorities can engage neighbouring residents on tips to enhance their safety as pedestrians. With at least five primary schools in the vicinity of Harehills Lane, it will also be useful to educate the children from these schools on road safety. For cyclists, local authorities can explore building cycling infrastructures such as dedicated cycling lanes along stretches with high propensity of cyclist accidents, such as Kirkstall Road and Headingley Lane. If cycling infrastructures already exist, local authorities can review them to improve the infrastructure. Given that pedestrians and cyclists are often victims in accidents with motor vehicles, more traffic enforcement can be conducted along these stretches to shape drivers' behaviours and deter egregious driving.
...
In this section, we will model accident rates for pedestrians and cyclists in Leeds by Middle Layer Super Ouput Areas (MSOAs) using various input variables. The central aim is to gain an understanding on how accident rates in a specific region could be correlated with various social and demographic factors.

## 7.1 Methodology

Using the origin-destination (OD) data from the 2011 Census available in the `pct` package, we ascertained the percentages of individuals who walk or cycle to work. These walking or cycling levels by MSOAs will be used as variables to predict pedestrian or cyclist accident rates. In this analysis, "accident rate" is defined as the number of accidents per 1,000 residents in a specified region. To compute accident rates, the total number of accidents recorded in each MSOA from 2014 to 2018 is divided by the population of the MSOA^[For instance, with a population size of 6,909 and a total pedestrian accident count of 16, the MSOA with the code "E02002331" has an accident rate of 2.32 per 1000 residents.]. The 2018 population numbers were used as proxies in estimating the population size of each MSOA. 

We explored two models---linear regression and random forest---to predict accident rates for pedestrians and cyclists. We first divided the 107 observations into training and test datasets based on an 80:20 split. The models were trained using the training data, and predictions were generated with the fitted models using the test data. A metric known as the "root-mean-squared-error" (RMSE)^[RMSE is a measure of how close the predicted values are from the actual values.] was used to evaluate the predictions by comparing them against the actual accident rates in the test data.

Next, we added further complexities to the modelling by introducing two more predictor variables---population density and total annual income. With three predictor variables, there are seven possible combinations, as shown in Appendix B. We trained the models on these combinations. As predictions vary depending on the random splitting of the original dataset, we performed cross-validation to decide which model and combination of input variables best predict accident rates for both pedestrians and cyclists.
...
Table 2 summarises the results of the performance of the two models in predicting pedestrian and cyclist accident rates of the MSOAs in the test data using walking and cycling levels respectively. The lower the RMSE, the closer the predictions were to the actual accident rates. These results show that the random forest model performed better in predicting pedestrian accident rates, but worse in predicting cyclist accident rates. 

```{r df_simple_model_results, include=TRUE, eval=TRUE}
...
different roads with same name.
...
roads recorded high accidents not because it is unsafe but because they have higher usage. normalised against flow/network data?
